...
public class SendMessageServlet extends  BaseServlet { 
	...
	private Message createMessage() {
		logger.info("*****inside create message in sendmessage servlet");
		logger.info("*****inside create message in sendmessage servlet " + HomeServlet.gcmMessage.getSubject());
		Message message = null;
		GcmMessage gcmMessage = HomeServlet.gcmMessage;
		String dataType = gcmMessage.getDatatype();
		//if(dataType.equalsIgnoreCase(ServerConstants.FIRE_DATATYPE)) {
			String timeInLongData = gcmMessage.getTimeInLong();
			message = new Message.Builder()
			.addData(ServerConstants.SUBJECT_HEADER, gcmMessage.getSubject())
			.addData(ServerConstants.MESSAGE_HEADER, gcmMessage.getMessage())
			.addData(ServerConstants.DATE_HEADER, gcmMessage.getDate())
			.addData(ServerConstants.TIME_HEADER, gcmMessage.getTime())
			.addData(ServerConstants.IMAGEURL_HEADER, gcmMessage.getImageUrl())
			.addData(ServerConstants.DATATYPE_HEADER, dataType)
			.addData(ServerConstants.TIMEINLONG_HEADER, timeInLongData)
			.build();
			String isUpdated = SensorData.updateSensorDataSent(timeInLongData, "true");
			logger.info("*****inside create message in sendmessage servlet " + isUpdated);
		//} 
		/*else if (dataType.equalsIgnoreCase(ServerConstants.MOTION_DATATYPE)) {
			String timeInLongData = gcmMessage.getTimeInLong();
			message = new Message.Builder()
			.addData(ServerConstants.SUBJECT_HEADER, gcmMessage.getSubject())
			.addData(ServerConstants.MESSAGE_HEADER, gcmMessage.getMessage())
		...
		HomeServlet.reset();
		logger.info("*****inside create message in sendmessage servlet after reset " + HomeServlet.gcmMessage.getSubject());
...
//		Sender sender = new Sender("AIzaSyAneC4ujZg0Stbt1UTfXLgldBBu_gbBiyQ");
		Sender sender = new Sender("AIzaSyATcrqTxfjzy_XwuhqMCjdafrzRfta4aQ4");
